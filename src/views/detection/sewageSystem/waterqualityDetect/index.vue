  ...
  export default {
    ...
    methods: {
      ...
      getwaterdata(id, index) {
        ...
        // 获取全部监测点
        this.$get("checkPoint/get", {type: 2, used: 2})
          .then(res => {
            if (res) {
      ...
      // 获取数据列表
      getlist () {
        this.$get("checkPointData/getList", {'checkPointId':this.id, 'type':'2',limit: this.pagination.pageSize, page: this.pagination.current,...this.quarydata})
          .then(res => {
            if (res) {
      ...
      add () {
        // 获取全部监测点
        this.$get("checkPoint/get", {type: 2})
          .then(res => {
            if (res) {
