      ...
      <a-layout-content>
        <a-card :body-style="{padding: '0 10px'}">
          ...
          <a-tabs
                  ...
                  :tabBarGutter="0"
          >
            <a-tab-pane tab="科室审核通过" :key="7" :closable=false>
              <div class="item">
<!--                <Ltable @rowDetails="rowDetails" :status="7" :columns="columns1" :scroll="{ y: 'calc(100vh - 380px)', x: 1330}" :getlistdata="{department: '9,14,15'}"></Ltable>-->
              </div>
            </a-tab-pane>
            <a-tab-pane tab="不下发" :key="-1" :closable=false>
              <div class="item">
              </div>
            </a-tab-pane>
          </a-tabs>
          <Ltable @rowDetails="rowDetails" :status="activeKey" :columns="activeKey===7?columns1:columns2"></Ltable>
        </a-card>
      </a-layout-content>
